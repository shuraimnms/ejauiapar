...
export const Route = createFileRoute("/conferences/past")({
  head: () => ({
    meta: [
      { title: "Past Conferences (2015 – 2026) — EJAUIAPAR" },
      { name: "description", content: "Archive of every EJAUIAPAR international conference and its published proceedings, from 2015 to 2026." },
      { property: "og:title", content: "Past Conferences — EJAUIAPAR" },
      { property: "og:description", content: "Year-wise tree of EJAUIAPAR conferences, venues, and conference proceedings volumes." },
    ],
...
const THEMES = [
  "Contemporary Issues in Ayurvedic, Unani and Interdisciplinary Pharmaceuticals & Allopathic Review",
...
function build(): YearBlock[] {
  ...
  for (let year = 2015; year <= 2026; year++) {
    ...
    const conferences: Conf[] = Array.from({ length: count }, (_, i) => ({
      n: i + 1,
      title: `EJAUIAPAR International Conference on ${THEMES[(i + year) % THEMES.length]}`,
  ...
  return (
    <SiteLayout>
      <PageHero
        ...
        intro="A complete archive of EJAUIAPAR international conferences, symposia, and their published proceedings — organised year-wise from the journal's launch year."
      />
        ...
        <div className="grid gap-6 p-6 lg:grid-cols-[minmax(0,3fr)_minmax(0,2fr)]">
          ...
          <div className="rounded-lg border border-border bg-secondary/40 p-5">
            ...
            <p className="mt-4 text-xs text-muted-foreground">
              Selected papers from each edition undergo double-blind peer review and are published as an EJAUIAPAR proceedings volume with individual DOIs.
            </p>
          </div>
        </div>
      ) : null}
