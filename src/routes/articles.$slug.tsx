...
export const Route = createFileRoute("/articles/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: loaderData
      ? [
          { title: `${loaderData.title} — EJAUIAPAR` },
...
function Article() {
  ...
  const apa = `${(a.authors as string[]).join(", ")} (${issue?.year ?? new Date(a.published_at ?? Date.now()).getFullYear()}). ${a.title}. European Journal of Ayurvedic, Unani and Interdisciplinary Pharmaceuticals & Allopathic Review, ${issue?.volume ?? "-"}(${issue?.number ?? "-"}), ${a.page_start ?? "-"}–${a.page_end ?? "-"}.${a.doi ? " https://doi.org/" + a.doi : ""}`;

  const bibtex = `@article{ejauiapar${issue?.year ?? ""}${slug.split("-")[0]},
  title = {${a.title}},
  author = {${(a.authors as string[]).join(" and ")}},
  journal = {European Journal of Ayurvedic, Unani and Interdisciplinary Pharmaceuticals & Allopathic Review},
  volume = {${issue?.volume ?? ""}},
  number = {${issue?.number ?? ""}},
